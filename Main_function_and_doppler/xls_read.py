...
class tx_xls_read():
    ...
    def __init__(self, file, t_start, t_stop):
        # 从file加载第一个工作表
        wb = xlrd.open_workbook(file)
        sheet = wb.sheet_by_index(0)
        GPS_tx_list_lat = sheet.col_values(8, t_start - 1, t_stop)             # 获取第9列的纬度
        GPS_tx_list_long = sheet.col_values(9, t_start - 1, t_stop)            # 获取第10列的经度
        ...
        v_tx_list = sheet.col_values(4, t_start - 1, t_stop)                   # 获取第5列的速度
...
class rx_xls_read():
    ...
    def __init__(self, file, t_start, t_stop):
        ...
        wb = xlrd.open_workbook(file)
        sheet = wb.sheet_by_index(0)
        GPS_rx_list_lat = sheet.col_values(9, t_start - 1, t_stop)             # 获取第9列的纬度
        GPS_rx_list_long = sheet.col_values(10, t_start - 1, t_stop)           # 获取第10列的经度
        ...
        v_rx_list = sheet.col_values(5, t_start - 1, t_stop)
        self.v_rx = np.transpose([np.array(v_rx_list)])
        
        self.T = sheet.col_values(0, t_start - 1, t_stop)
